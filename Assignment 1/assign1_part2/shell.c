...
{
   pid_t shell_pid, pid_from_fork, pid2;
   ...
   int counter = 1; //counter for number of command lines entered 
   char cmd_hist[9][256]; //2d array for command history storing
   int depth = 0; //depth counter for subshells
   ...
   /* The Shell runs in an infinite loop, processing input. */
      ...
      if(n_read>1)
         counter++; //update counter if we have at least 1 character
       ...
       * we want to run.                                                */
         ...
         if(counter-2<9)
            strcpy(cmd_hist[counter-2], buffer);

         if(buffer[0]=='!'){ // detect if user wants a previous command
         int cmd = 0;	//flag variable
         for(int i =1; i<counter-1; i++){ //checks for numbers 1 through current line-1
            if(buffer[1]==i+48){ //comparing number's ascii value
               strncpy(buffer, cmd_hist[i-1], 256); //copy i-1th command back to buffer so we can run it again
               cmd = 1;							// update cmd so "Not Valid" if statement is blocked
            }
         }
         if(cmd == 0 && counter>2){ //invalid number is entered
      ...
      if (!strcmp(exec_argv[0], "ls")) //if user enters ls
         exec_argv[0] = "/bin/ls";     // replace with /bin/ls 
      ...
      /* Execute Commands */
         ...
         pid_from_fork = fork(); //fork a new child process
         ...
         if (pid_from_fork < 0) {
          ...
          if(pid_from_fork==0){ // we are the child
         	if(!strcmp(exec_argv[0], "sub")){ //if user enters sub command
         		counter=1; //update counter for child
         		depth++;	//increase depth in case we're too deep
         		shell_pid= getpid();	//get pid of process
         		if(depth>=3){	//illegal move, block user from going deeper
         			fprintf(stderr, "Too deep!\n");
         			return 0;
         		}
         	}
         	else //let the child process run
