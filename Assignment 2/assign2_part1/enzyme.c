...
void *run_enzyme(void *data) {
	...
	*/
	//printf("run\n");
	thread_info_t* curThread = (thread_info_t*)data;
	curThread->swapcount=0;
	pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);
	
	if(curThread->string[0]==67) //comparing to ascii value of C
		pthread_cancel(pthread_self());
	
	while(please_quit==0){
		if(curThread->string[0]>curThread->string[1]){
			workperformed=1;
			curThread->swapcount++;
			int temp = curThread->string[0];
			curThread->string[0]=curThread->string[1];
			curThread->string[1]=temp;
		}
		if(use_yield!=0){
			sched_yield();
		}
...
int join_on_enzymes(pthread_t *threads, int n) {
	...
	if ((void*)status == PTHREAD_CANCELED) {
	    ...
	    } else {	//I think status holds the terminated thread's data
...
int smp2_main(int argc, char **argv) {
	...
	strncpy(string,argv[1],MAX); // Why is this necessary? Why cant we give argv[1] directly to the thread functions?
								 // Because argv[1] could have over 100 characters
	please_quit = 0;
	use_yield =1;
