...
void update_run_time(thread_info_t *info) {
	...
	clock_gettime(CLOCK_REALTIME, &(info->suspend_time)); //issues?
...
void update_wait_time(thread_info_t *info) {
        /* TODO: implement this function */
	clock_gettime(CLOCK_REALTIME, &(info->resume_time)); //issues?
...
static void init_sched_queue(int queue_size)
{
	...
	timer_create(CLOCK_REALTIME, &sevp, &timer); //error?
}
...
static void suspend_worker(thread_info_t *info)
{
	...
	if(info->quanta>0) {
	   ...
	   */
		...
		act.sa_sigaction = suspend_thread;
...
void setup_sig_handlers() {
	...
	act2.sa_sigaction = cancel_thread;
	...
	act3.sa_sigaction = suspend_thread;
...
static void *scheduler_run(void *unused)
{
	...
	new_value.it_interval.tv_nsec = (1/1000000000)*QUANTUM;
	new_value.it_value.tv_sec = QUANTUM;
	new_value.it_value.tv_nsec = (1/1000000000)*QUANTUM; //issues?
